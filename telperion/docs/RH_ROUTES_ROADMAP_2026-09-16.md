...
| A1b | **DONE — clause REFUTED** (2026-09-19) | — | — | GL(2) Satake extension of B-mult-twisted. `L(s,Δ)` joined the zoo and the clause REJECTS it, as predicted. Kernel result is an **IFF**, stronger than the predicted rejection: the clause's amplitude-generation law holds for a degree-2 prime layer **iff the Satake determinant vanishes**, i.e. exactly on the GL(1) fiber (`SatakeDegreeTwo.scalarGenerated_powerSum_iff`, axiom-clean; node `MM_satake_degree_two_rejects_delta`, NOT granted — awaiting blind read-back). So (B-mult-twisted) is **false** as an arithmetic-class predicate: it encodes the degree-1 coincidence `a_p = α_p`. Repair identified and costed (degree-d Satake power sums; composite-vanishing half survives unchanged) but **does not** repair the classification conjecture — that remains bounded below by A1e. |
| A1b-dk | **DONE (2026-09-23)** | -- | A1b | Dedekind product-closure control: `zeta_K` for `Q(sqrt(-5))` = `zeta * L(chi_-20)` joins the zoo; both factors PASS (B-mult-twisted), the product FAILS. No Deligne input. Kernel: `DedekindQuadratic.lean`. Any repaired clause is checked against `zeta_K` first. QC_AXIOMS_DRAFT appendix v5. |
...
**A1b OUTCOME (2026-09-19, kernel).** The falsification landed and is sharper than the roadmap predicted. The clause was expected to reject `L(s,Δ)`; what is proved is the *exact* characterization — a degree-2 layer satisfies (B-mult-twisted)'s generation law **iff `α_p β_p = 0`**. Since unitary normalization forces `α_p β_p = 1`, EVERY GL(2) form is rejected, with an `m = 2` amplitude defect of exactly `2 α_p β_p` that does not shrink at any prime for any form. ζ and `L(s,χ)` pass only because they sit on the degenerate `β = 0` fiber (`deg1_scalarGenerated`, the anti-vacuity witness). Δ's Satake determinant is re-derived in-kernel from `Δ = q∏(1−qⁿ)²⁴` — τ is computed, never quoted — and cross-checked against the Hecke recursion and coprime multiplicativity, both of which fire when the η-exponent is corrupted. Ramanujan–Petersson (Deligne) is a NAMED input used only to certify that the rejected object is tempered and therefore a genuine member of the class; no theorem consumes it. Consequence for the route: **A1c and A1d inherit a broken premise** — both were scoped against a clause that is now known false, so their statements need re-basing on the repaired (degree-d Satake) clause before either is attacked. `conjecture1_proved = False`.
