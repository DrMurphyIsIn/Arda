...
**Vocabulary** (AUTHORED block in `missions/rh/lean/Statements/RHDefs.lean`; `WeilExplicit` already
exists there and is reused unchanged, `WeilForm` is mirrored from the `weil_form_enclosure` island):
...
| `rayleigh_gram` | one Rayleigh direction | a single direction, not an infimum. |
