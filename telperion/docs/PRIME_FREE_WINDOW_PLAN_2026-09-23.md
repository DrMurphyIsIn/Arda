...
Exact u-space form of the same functional (verified numerically to 1e-7, NOT yet in Lean):
...
Stage 1 (narrow support, PROVE with explicit constants).  r-space dominance: |ĝ(r)|^2 <= ∫|f|
<= 4 L f(0) (pointwise |f(u)| <= f(0) by AM-GM under the integral, supp f ⊆ [-2L, 2L]),
∫ |ĝ|^2 = 2 pi f(0) (E6Bridge4 `inversion_zero` applied to f), pole terms >= -8 L e^L f(0),
and a layer cake over the E6Bridge30 floors (generic weight version of E6Bridge30 `Cake`):
(1/2pi) ∫ |ĝ|^2 psiR >= f(0) [phi_top - (4L/pi) sum_k (phi_k - phi_{k-1}) rho_k].  With the 20
floors (base psiR(0) >= -4.2315, band sum 24.67252) the bound is positive for L <= 0.0300; the
theorem is stated at L₁ = 1/40 (margin 0.200 of ||g||^2).
The Dirichlet u-space route would reach L ~ 0.066 and the exact threshold is ~ 0.36; the r-space
route is chosen because every analytic input already exists on the island.
