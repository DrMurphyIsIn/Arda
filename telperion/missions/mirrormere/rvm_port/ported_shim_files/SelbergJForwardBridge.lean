...
-- PORT SHIM (v4.33-rc2 -> v4.32): `Set.mem_ofPred_eq` was `Set.mem_setOf_eq` at v4.32
-- (renamed later). Alias restores the source name; identical `rfl` statement.
namespace Set
theorem mem_ofPred_eq {α : Type*} {x : α} {p : α → Prop} : (x ∈ {y | p y}) = p x := rfl
end Set
