...
-- PORT SHIM (v4.33-rc2 -> v4.32): Mathlib gained `Complex.div_cpow_ofReal_nonneg`
-- after v4.32.  Provided here from v4.32 primitives (mul_cpow_ofReal_nonneg +
-- inv_cpow_eq_ite + arg_ofReal_of_nonneg).  Proof-carrying, zero-axiom-drift.
namespace Complex
theorem div_cpow_ofReal_nonneg {a b : ℝ} (ha : 0 ≤ a) (hb : 0 ≤ b) (r : ℂ) :
    ((a : ℂ) / (b : ℂ)) ^ r = (a : ℂ) ^ r / (b : ℂ) ^ r := by
  rcases eq_or_lt_of_le hb with (rfl | hb')
  · rcases eq_or_ne r 0 with (rfl | hr)
    · simp
    · simp [Complex.ofReal_zero, div_zero, Complex.zero_cpow hr]
  · have hb0 : (b : ℂ) ≠ 0 := Complex.ofReal_ne_zero.mpr hb'.ne'
    rw [div_eq_mul_inv, div_eq_mul_inv, ← Complex.ofReal_inv,
        Complex.mul_cpow_ofReal_nonneg ha (inv_nonneg.mpr hb),
        Complex.ofReal_inv, Complex.inv_cpow_eq_ite]
    split_ifs with h
    · exfalso
      rw [Complex.arg_ofReal_of_nonneg hb] at h
