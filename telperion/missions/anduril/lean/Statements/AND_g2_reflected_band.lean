-- DO NOT EDIT BY HAND — generated by telperion mission; node AND_g2_reflected_band; sha256 871cfd73a03b6c35
-- VERBATIM contiguous span of ReflectedBand_t14.lean:29-47 (v4.32 zeta_reflection
-- island, namespace ReflectedBand_t14) in artifact order: def d (band-data literal),
-- theorem ok (the kernel-decided sign chain -- the G2 headline), def grid, and the
-- pilot band conclusion they feed.  The intervening ok/grid are included verbatim so
-- the grant gate's single-span containment matches (2026-09-17 pre-flight fix); the
-- namespace is left open to end-of-file so the span ends at pilot.
import Mathlib
