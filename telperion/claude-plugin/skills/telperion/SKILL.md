...
- **Supply vs. auto:** Nullstellensatz / Consequence / Infeasibility
  ...
  Telperion FINDS it: `find_handelman_certificate` (exact LP, sympy-only, in the
  quick group) and the SDP finders `find_putinar_certificate` (`sos_sdp.py`),
  `find_sos_refutation` and `find_real_nullstellensatz` (`sdp_finder.py`) —
  numeric SDP rounded to exact rationals, in the cvxpy-gated `sdp` group. Found or
  supplied, the certificate is re-verified exactly and the kernel checks it — a
  finder miss is a refusal, never a false theorem.
