/-  Probes/ArbEconomics_T10000.lean -- KERNEL-CHECKED enclosure of zeta(1/2 + 10000 i) (GENERATED; do not edit).

    `zeta_re`, `zeta_im` bound Re / Im of riemannZeta (1/2 + 10000 i) with NO hypotheses:
      * the Dirichlet sum over n = 1..125499 is kernel-evaluated in 251 `decide +kernel` chunks
        (parts P0..P6) and composed by `ArbEcon.chunk_sound` (ArbEconomics_Sound);
      * the EM order-3 tail at N = 125500 is `EMZetaTail.em_zeta_critical_line3_enclosure`
        (via `ArbEcon.em3_remainder_le`, E = Q / (180 N^2 r), Q = 1000000043750, r = 354);
      * `ArbEcon.zeta_re_bounds` / `zeta_im_bounds` (ArbEconomics_Zeta) assemble the result.
    Certified: Re in [-0.3403714, -0.33837621], Im in [-0.0380891, -0.03609391] (width dominated by E).
    conjecture1_proved = False.  One height, finite interval arithmetic; nothing about RH.
-/
import Probes.ArbEconomics_Zeta
import Probes.ArbEconomics_T10000_P0
import Probes.ArbEconomics_T10000_P1
import Probes.ArbEconomics_T10000_P2
import Probes.ArbEconomics_T10000_P3
import Probes.ArbEconomics_T10000_P4
import Probes.ArbEconomics_T10000_P5
import Probes.ArbEconomics_T10000_P6

namespace ArbEcon.I_T10000

theorem valid : ArbEcon.Valid cfg (10000 : ℝ) 9 where
  one_eq := by decide
  two1_eq := by decide
  oneSq_eq := by decide
  t_eq := by show (10000 : ℝ) = ((10000 : ℕ) : ℝ) / 2 ^ (0 : ℕ); norm_num
  pi_ball := by
