/-  RS4_Sound.lean -- lane B4: SOUNDNESS of the Riemann-Siegel main-term checker `RS4.check`.

    ## The headline

      `RS4.check_sound` : for an evaluator configuration `c` valid at height `t`
      (`ArbEcon.Valid c t K`, i.e. `t = tn / 2^tq` and the pi / Horner / log-kernel facts) and any
      certificate `z`, if the KERNEL accepts (`check c z = true`) then
        * `10000 <= t`, `rsNn t = z.N`, `0 < rsFrac t`, `cos (2 pi rsFrac t) ≠ 0`, and
        * for EVERY phase `phi` with `|phi - rsThetaMain t| <= 1/t`,
              z.zlo / 2^(2P) <= Zmain t phi <= z.zhi / 2^(2P),
          where `Zmain t phi = 2 sum_{n <= N} n^(-1/2) cos(phi - t log n)
                                + (-1)^(N+1) (t/2pi)^(-1/4) rsPsi (rsFrac t)`
          is VERBATIM the main term of `RSInt.rs_Z_C0`.

    The hypotheses of `rs_Z_C0` (`t >= 10000`, `0 < p`, `cos 2 pi p ≠ 0`) are OUTPUTS of the checker,
    so `RS4_Z` combines the two into a hypothesis-free enclosure of Hardy's Z.
...
/-- The Riemann-Siegel main term with phase `phi` (verbatim the main term of `RSInt.rs_Z_C0`). -/
def Zmain (t φ : ℝ) : ℝ :=
  2 * ∑ n ∈ Finset.Icc 1 (RSInt.rsNn t), (n : ℝ) ^ (-(1 / 2 : ℝ)) * Real.cos (φ - t * Real.log n) +
    (-1 : ℝ) ^ (RSInt.rsNn t + 1) * (t / (2 * π)) ^ (-(1 / 4 : ℝ)) * RSInt.rsPsi (RSInt.rsFrac t)

/-! ## 0. small real facts -/

/-- The truncated alternating log series `L(y) = y - y^2/2 + ... - y^8/8`. -/
def Lser (y : ℝ) : ℝ :=
  y - y ^ 2 / 2 + y ^ 3 / 3 - y ^ 4 / 4 + y ^ 5 / 5 - y ^ 6 / 6 + y ^ 7 / 7 - y ^ 8 / 8

theorem log_one_add_near {y : ℝ} (h0 : 0 ≤ y) (h1 : y ≤ 1 / 2) :
    |Real.log (1 + y) - Lser y| ≤ 2 * y ^ 9 := by
  have hy : |(-y)| < 1 := by rw [abs_neg, abs_of_nonneg h0]; linarith
  have h := Real.abs_log_sub_add_sum_range_le hy 8
  have hs : (∑ i ∈ Finset.range 8, (-y) ^ (i + 1) / ((i : ℝ) + 1)) = -Lser y := by
    simp only [Finset.sum_range_succ, Finset.sum_range_zero, Lser]; push_cast; ring
  rw [hs, sub_neg_eq_add, abs_neg, abs_of_nonneg h0] at h
  rw [show Real.log (1 + y) - Lser y = -Lser y + Real.log (1 + y) by ring]
  refine le_trans h ?_
  have hpos : 0 < 1 - y := by linarith
  rw [div_le_iff₀ hpos]
  have : 0 ≤ y ^ 9 := by positivity
  rw [show (8 : ℕ) + 1 = 9 from rfl]
  nlinarith [mul_nonneg this (by linarith : (0 : ℝ) ≤ 1 / 2 - y)]

theorem lp_real (Y o : ℕ) (ho : 0 < o) :
    ((lpPos Y o : ℕ) : ℝ) - (lpNeg Y o : ℕ) = 840 * (o : ℝ) ^ 9 * Lser ((Y : ℝ) / o) := by
  have hoR : (o : ℝ) ≠ 0 := by exact_mod_cast ho.ne'
  unfold lpPos lpNeg Lser
  push_cast
  field_simp
  ring

theorem sgnI_cast (b : Bool) (m : ℕ) : ((sgnI b m : ℤ) : ℝ) = sgn b * m := by
  cases b <;> simp [sgnI, sgn]

theorem two_zpow_negP (P : ℕ) : (2 : ℝ) ^ (-(P : ℤ)) = 1 / 2 ^ P := by
  rw [zpow_neg, zpow_natCast, one_div]

/-- A signed ball `|x 2^P - m| <= r` is DIntv membership in `ballD P m r`. -/
theorem ballD_mem {P : ℕ} {x : ℝ} {m : ℤ} {r : ℕ} (h : |x * 2 ^ P - m| ≤ r) :
    (ballD P m r).memR x := by
  have hP : (0 : ℝ) < 2 ^ P := by positivity
  unfold ballD DIntv.memR
  simp only [two_zpow_negP]
  rw [abs_le] at h
  push_cast
  constructor
  · rw [← mul_div_assoc, mul_one, div_le_iff₀ hP]; linarith
  · rw [← mul_div_assoc, mul_one, le_div_iff₀ hP]; linarith

/-- A Nat/Int bracket `lo <= x 2^P <= hi` is DIntv membership at exponent `-P`. -/
theorem brD_mem {P : ℕ} {x : ℝ} {lo hi : ℤ} (h0 : (lo : ℝ) ≤ x * 2 ^ P) (h1 : x * 2 ^ P ≤ hi) :
    DIntv.memR x ⟨lo, hi, -(P : ℤ)⟩ := by
  have hP : (0 : ℝ) < 2 ^ P := by positivity
  unfold DIntv.memR
  simp only [two_zpow_negP]
  constructor
...
theorem negIf_mem {x : ℝ} {I : DIntv} (b : Bool) (h : I.memR x) :
    (negIf b I).memR (if b then x else -x) := by
  cases b
  · have := DIntv.neg_sound h
    simpa [negIf, DIntv.neg] using this
  · simpa [negIf] using h
...
theorem t_ge (hv : Valid c t K) (hk : Checks c z) : 10000 ≤ t := by
  rw [hv.t_eq, le_div_iff₀ (by positivity)]
  have := hk.ht
  exact_mod_cast this

theorem t_pos (hv : Valid c t K) (hk : Checks c z) : 0 < t := by
  linarith [t_ge hv hk]
...
theorem alpha_key (hv : Valid c t K) (hk : Checks c z) :
...
theorem A0_le (hv : Valid c t K) (hk : Checks c z) :
...
theorem A1_ge (hv : Valid c t K) (hk : Checks c z) :
...
theorem N_lt_alpha (hv : Valid c t K) (hk : Checks c z) : (z.N : ℝ) < RSInt.rsAlpha t := by
  have hP : (0 : ℝ) < 2 ^ c.P := by positivity
...
theorem alpha_lt_N1 (hv : Valid c t K) (hk : Checks c z) : RSInt.rsAlpha t < (z.N : ℝ) + 1 := by
  have hP : (0 : ℝ) < 2 ^ c.P := by positivity
...
theorem N_eq (hv : Valid c t K) (hk : Checks c z) : RSInt.rsNn t = z.N := by
  unfold RSInt.rsNn
  rw [Nat.floor_eq_iff (alpha_nonneg t)]
  exact ⟨(N_lt_alpha hv hk).le, alpha_lt_N1 hv hk⟩

theorem frac_eq (hv : Valid c t K) (hk : Checks c z) :
    RSInt.rsFrac t = RSInt.rsAlpha t - z.N := by
  unfold RSInt.rsFrac; rw [N_eq hv hk]

theorem frac_pos (hv : Valid c t K) (hk : Checks c z) : 0 < RSInt.rsFrac t := by
  rw [frac_eq hv hk]; linarith [N_lt_alpha hv hk]

theorem NO_le_A0 (hk : Checks c z) : z.N * c.one ≤ z.A0 := le_of_lt hk.hNlo

theorem NO_le_A1 (hv : Valid c t K) (hk : Checks c z) : z.N * c.one ≤ z.A1 := by
...
theorem p_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem p_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem N_pos (hk : Checks c z) : (0 : ℝ) < z.N := by
  have := hk.hN; exact_mod_cast this

/-- `y = p / N` bracket. -/
theorem y_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem y_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem y_nonneg (hv : Valid c t K) (hk : Checks c z) : 0 ≤ RSInt.rsFrac t / z.N :=
  div_nonneg (frac_pos hv hk).le (N_pos hk).le

theorem y_le_half (hv : Valid c t K) (hk : Checks c z) : RSInt.rsFrac t / z.N ≤ 1 / 2 := by
  have hP : (0 : ℝ) < 2 ^ c.P := by positivity
...
theorem log_alpha (hv : Valid c t K) (hk : Checks c z) :
...
/-- `LG0 <= log (1 + y) 2^P <= LG1`. -/
theorem lg_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem lg_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem dstate_inv (hv : Valid c t K) (hk : Checks c z) : Inv c t z.N (dstate c z) := by
  have h := run_sound c t K hv (z.N - 1) 1 (St.init c) (inv_init c t hv.one_eq)
  have hN := hk.hN
  rwa [show 1 + (z.N - 1) = z.N by omega] at h

theorem loga_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem loga_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem tn_pos (hk : Checks c z) : (0 : ℝ) < c.tn := by
  have h : ((10000 * 2 ^ c.tq : ℕ) : ℝ) ≤ c.tn := by exact_mod_cast hk.ht
  push_cast at h
  have : (0 : ℝ) < 10000 * 2 ^ c.tq := by positivity
  linarith

theorem phase_lo (hv : Valid c t K) (hk : Checks c z) {φ : ℝ}
...
theorem phase_hi (hv : Valid c t K) (hk : Checks c z) {φ : ℝ}
...
theorem alpha_pos (hv : Valid c t K) (hk : Checks c z) : 0 < RSInt.rsAlpha t := by
...
theorem w_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem w_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem U_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem U_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem G_bounds (hv : Valid c t K) (hk : Checks c z) :
...
theorem V_lo (hv : Valid c t K) (hk : Checks c z) :
...
theorem V_hi (hv : Valid c t K) (hk : Checks c z) :
...
theorem mainBox_mem (hv : Valid c t K) (hk : Checks c z) {φ : ℝ}
...
/-- **Soundness of the Riemann-Siegel main-term checker (lane B4).**  If the kernel accepts
    `(c, z)` for a configuration valid at height `t`, then `t >= 10000`, the Riemann-Siegel
...
theorem check_sound {c : Cfg} {t : ℝ} {K : ℕ} (hv : Valid c t K) (z : Cert)
    (h : check c z = true) :
    10000 ≤ t ∧ RSInt.rsNn t = z.N ∧ 0 < RSInt.rsFrac t ∧
      Real.cos (2 * π * RSInt.rsFrac t) ≠ 0 ∧
      ∀ φ : ℝ, |φ - RSInt.rsThetaMain t| ≤ 1 / t →
        (z.zlo : ℝ) / 2 ^ (2 * c.P) ≤ Zmain t φ ∧ Zmain t φ ≤ (z.zhi : ℝ) / 2 ^ (2 * c.P) := by
  have hk : Checks c z := checks_of (of_decide_eq_true h)
  have hP : (0 : ℝ) < 2 ^ c.P := by positivity
  have hN := N_eq hv hk
  -- the Psi quotient
