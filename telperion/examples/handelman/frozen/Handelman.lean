...
-- handelman_found_1mx2: Handelman certificate  p = Σ c_α ∏ ℓ_i^α (nonneg combination of constraint products) on the polytope.
theorem handelman_found_1mx2 : ∀ x y : ℝ, (0:ℝ) ≤ 1 - x → (0:ℝ) ≤ 1 + x → (0:ℝ) ≤ 1 - x ^ 2 := by
  intro x y h1 h2
  have t1 : (0:ℝ) ≤ 1 * (1 - x)^1 * (1 + x)^1 := mul_nonneg (mul_nonneg ((by norm_num : (0:ℝ) ≤ 1)) (pow_nonneg h1 1)) (pow_nonneg h2 1)
  have hid : (1 - x ^ 2 : ℝ) = 1 * (1 - x)^1 * (1 + x)^1 := by ring
  rw [hid]; linarith

-- handelman_found_box: Handelman certificate  p = Σ c_α ∏ ℓ_i^α (nonneg combination of constraint products) on the polytope.
theorem handelman_found_box : ∀ x y : ℝ, (0:ℝ) ≤ 1 - x → (0:ℝ) ≤ 1 + x → (0:ℝ) ≤ 1 - y → (0:ℝ) ≤ 1 + y → (0:ℝ) ≤ 2 - x ^ 2 - y ^ 2 := by
  intro x y ha hb hc hd
  have t1 : (0:ℝ) ≤ 1 * (1 - x)^1 * (1 + x)^1 := mul_nonneg (mul_nonneg ((by norm_num : (0:ℝ) ≤ 1)) (pow_nonneg ha 1)) (pow_nonneg hb 1)
  have t2 : (0:ℝ) ≤ 1 * (1 - y)^1 * (1 + y)^1 := mul_nonneg (mul_nonneg ((by norm_num : (0:ℝ) ≤ 1)) (pow_nonneg hc 1)) (pow_nonneg hd 1)
  have hid : (2 - x ^ 2 - y ^ 2 : ℝ) = 1 * (1 - x)^1 * (1 + x)^1 + 1 * (1 - y)^1 * (1 + y)^1 := by ring
  rw [hid]; linarith
