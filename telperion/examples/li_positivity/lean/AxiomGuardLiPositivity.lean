...
import LowHeightBox
import LiBoxRungs
import Probes.Dogfood_preordering_multiplier
...
-- Probes/Dogfood_preordering_multiplier (the Telperion `preordering_multiplier` emitter dogfood,
-- SHAPES_AUDIT_48H_2026-09-22 section 2 rank 3): LiBoxRungs' re_Q1..re_Q5_nonneg regenerated from
-- exact preordering certificates (the hand multipliers 1, 14 s, s^2, plus LP-found variants), each
-- a real-variable core + a complex face, and the kernel-checked REFUTATION of the N = 6 disk claim
-- the emitter refused (OBSTRUCTED_AND_LOCATED at (4/5, -2/5)).  Generated by
-- examples/li_positivity/dogfood_preordering_multiplier.py.  Holds no registry node.
-- Nothing here proves RH. conjecture1_proved = False.
