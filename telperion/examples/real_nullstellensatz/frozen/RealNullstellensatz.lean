...
-- real_nss_found_x_zero: Real-Nullstellensatz certificate  p^(2m) + s = Σ h_k·g_k (s a sum of squares) — p vanishes on the REAL variety.
theorem real_nss_found_x_zero : ∀ x y : ℝ, x ^ 2 + y ^ 2 = 0 → x = 0 := by
  intro x y e1
  have hpow : (0:ℝ) ≤ (x)^2 := by positivity
  have hsos : (0:ℝ) ≤ 1 * (y)^2 := by positivity
  have key : (x)^2 + (1 * (y)^2) = 0 := by linear_combination (1) * e1
...
-- real_nss_found_mult2: Real-Nullstellensatz certificate  p^(2m) + s = Σ h_k·g_k (s a sum of squares) — p vanishes on the REAL variety.
theorem real_nss_found_mult2 : ∀ x y : ℝ, x ^ 3 = 0 → x = 0 := by
  intro x y e1
  have hpow : (0:ℝ) ≤ (x)^4 := by positivity
  have hsos : (0:ℝ) ≤ 0 := by positivity
  have key : (x)^4 + (0) = 0 := by linear_combination (x) * e1
