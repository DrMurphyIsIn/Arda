...
def guard_module_name(top: int) -> str:
    return f"Guard_h{top}"


def emit_guard_module(top: int) -> str:
    """`Guard_h<top>.lean`: the block-top axiom battery as a MODULE, so a bare
    `lake build` of the block package runs it and FAILS on any axiom set other than
    the three standard ones (or on sorryAx).  `#guard_msgs (whitespace := lax)`
    compares the `#print axioms` message exactly (list wrapped across lines is fine).
    This replaces the `lake env lean <file>` guard step, which could not spawn lean
    on a hosted runner at block depth 2 ("could not execute external process",
    2026-09-17 #552 run) even though the block itself had built."""
    cap = f"AllZeros_h{top}.all_nontrivial_zeros_up_to_height_{top}_of_bands"
    return (
        f"-- GENERATED by campaign.py register-lakefile --sharded; DO NOT EDIT.\n"
        f"-- Block-top axiom battery for ZetaBands_h{top}: `lake build` fails unless the\n"
        f"-- capstone depends on exactly [propext, Classical.choice, Quot.sound].\n"
        f"-- conjecture1_proved = False (finite localization to height {top}, not RH).\n"
        f"import AllZeros_h{top}\n"
        f"/-- info: '{cap}' depends on axioms: [propext, Classical.choice, Quot.sound] -/\n"
...
def register_lakefile_sharded(t_from: int, t_to: int, segments: bool,
    ...
    if segments:
        ...
        # block-top axiom battery module (see emit_guard_module); the block-top
        # capstone AllZeros_h<block top> exists iff the segment tops reach it.
...
def main() -> int:
    ...
    return {"plan": cmd_plan, "emit-bands": cmd_emit_bands,
            ...
            "emit-segment": cmd_emit_segment,
            "emit-indexed": cmd_emit_indexed, "status": cmd_status,
