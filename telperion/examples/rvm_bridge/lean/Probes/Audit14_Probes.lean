...
/- ===== AUDIT ADDITIONS (blind auditor) ===== -/
#print axioms threshold_example_le
...
example {ρ₀ : ℂ} (h₀ : IsNontrivialZero ρ₀) {D xmin : ℝ}
    (hmax : ∀ ρ : ℂ, IsNontrivialZero ρ → |ρ.im - ρ₀.im| ≤ D → |1 / 2 - ρ.re| ≤ |1 / 2 - ρ₀.re|)
    ...
    (∑ ρ ∈ zeroWindow ρ₀.im D, term ρ₀.im lam ρ).re
      ≤ -((1 / 2 - ρ₀.re) ^ 2 * Real.exp (2 * lam * (1 / 2 - ρ₀.re) ^ 2))
        + (D ^ 2 + 1 / 4) * Real.exp (2 * lam * ((1 / 2 - ρ₀.re) ^ 2 - xmin ^ 2))
          * ∑ ρ ∈ zeroWindow ρ₀.im D, (WeilExplicit.zeroMult ρ : ℝ) :=
  re_window_sum_le h₀ hmax (fun ρ hnt hw hne => ?_) hlam hD hx0

/- A2. hmax deleted: the competitor's exponent cannot be bounded by Y (expected FAIL). -/
example {ρ₀ : ℂ} (h₀ : IsNontrivialZero ρ₀) {D xmin : ℝ}
    (hsep : ∀ ρ : ℂ, IsNontrivialZero ρ → |ρ.im - ρ₀.im| ≤ D → ρ.im ≠ ρ₀.im → xmin ≤ |ρ.im - ρ₀.im|)
    {lam : ℝ} (hlam : 0 ≤ lam) (hD : 0 ≤ D) (hx0 : 0 ≤ xmin) :
    (∑ ρ ∈ zeroWindow ρ₀.im D, term ρ₀.im lam ρ).re
      ≤ -((1 / 2 - ρ₀.re) ^ 2 * Real.exp (2 * lam * (1 / 2 - ρ₀.re) ^ 2))
        + (D ^ 2 + 1 / 4) * Real.exp (2 * lam * ((1 / 2 - ρ₀.re) ^ 2 - xmin ^ 2))
          * ∑ ρ ∈ zeroWindow ρ₀.im D, (WeilExplicit.zeroMult ρ : ℝ) :=
  re_window_sum_le h₀ (fun ρ hnt hw => ?_) hsep hlam hD hx0

/- A3. Independent check of the two threshold consequences and the closing inequality
   (expected SUCCESS, axiom-clean). -/
theorem audit_closing (y0 Y E C T : ℝ) (hy0 : 0 < y0) (hYy : y0 ^ 2 ≤ Y) (hE : 0 < E)
    (hmain : -(Y * E) ≤ -(y0 ^ 2 * E)) (hcomp : C ≤ y0 ^ 2 / 4 * E) (htail : T ≤ y0 ^ 2 / 4 * E) :
    -(Y * E) + C + T < 0 := by
  have : 0 < y0 ^ 2 * E := by positivity
  linarith
#print axioms audit_closing

/- A4. The role of `max 1`: le_exp_of_log_le holds for Q < 1 and for Q ≤ 0 (expected SUCCESS). -/
example (lam : ℝ) (h : Real.log (max 1 (1 / 2 : ℝ)) / (2 * 1) ≤ lam) :
    (1 / 2 : ℝ) ≤ Real.exp (2 * lam * 1) := le_exp_of_log_le one_pos h
example (lam : ℝ) (h : Real.log (max 1 (-3 : ℝ)) / (2 * 1) ≤ lam) :
    (-3 : ℝ) ≤ Real.exp (2 * lam * 1) := le_exp_of_log_le one_pos h

/- A5. Consumption: monotonicity in B, the unbounded-threshold theorem, and the instrument's
   contradiction step re-composed (expected SUCCESS). -/
example (N : ℕ) (xmin D : ℝ) : effectiveThreshold (1/10) xmin N 1 D ≤ effectiveThreshold (1/10) xmin N 2 D :=
  effectiveThreshold_mono_B N (by norm_num) (by norm_num)
example (B : ℝ) : ∃ xmin : ℝ, 0 < xmin ∧ 10 ^ 9 ≤ effectiveThreshold (1 / 2) xmin 1 B 1 :=
  effectiveThreshold_unbounded_of_small_spacing (by norm_num) le_rfl le_rfl le_rfl B _
example {ρ₀ : ℂ} (h₀ : IsNontrivialZero ρ₀) {y0 D xmin : ℝ} (hy0 : 0 < y0) (hy : y0 ≤ |1 / 2 - ρ₀.re|)
    (hD : 1 ≤ D) (hx : 0 < xmin)
    (hmax : ∀ ρ : ℂ, IsNontrivialZero ρ → |ρ.im - ρ₀.im| ≤ D → |1 / 2 - ρ.re| ≤ |1 / 2 - ρ₀.re|)
    (hsep : ∀ ρ : ℂ, IsNontrivialZero ρ → |ρ.im - ρ₀.im| ≤ D → ρ.im ≠ ρ₀.im → xmin ≤ |ρ.im - ρ₀.im|)
    {N : ℕ} (hN : ∑ ρ ∈ zeroWindow ρ₀.im D, WeilExplicit.zeroMult ρ ≤ N) {Bmax : ℝ}
    (hB : constB ρ₀.im ≤ Bmax)
    (hpos : 0 ≤ (zeroSide (gaussTest ρ₀.im (effectiveThreshold y0 xmin N Bmax D))).re) : False := by
  have := effective_gaussian_dominance h₀ hy0 hy hD hx hmax hsep hN
    (effectiveThreshold_mono_B N hy0 hB)
  linarith

/- A6. The threshold hypothesis is load-bearing: at lam = 1 with these parameters the theorem
   does not apply (expected FAIL: 299.57 ≤ 1 is false). -/
example {ρ₀ : ℂ} (h₀ : IsNontrivialZero ρ₀) (hy : 1 / 10 ≤ |1 / 2 - ρ₀.re|)
    (hmax : ∀ ρ : ℂ, IsNontrivialZero ρ → |ρ.im - ρ₀.im| ≤ 2 → |1 / 2 - ρ.re| ≤ |1 / 2 - ρ₀.re|)
    (hsep : ∀ ρ : ℂ, IsNontrivialZero ρ → |ρ.im - ρ₀.im| ≤ 2 → ρ.im ≠ ρ₀.im → 1 / 5 ≤ |ρ.im - ρ₀.im|)
    (hN : ∑ ρ ∈ zeroWindow ρ₀.im 2, WeilExplicit.zeroMult ρ ≤ 20) (hB : constB ρ₀.im ≤ 1) :
    (zeroSide (gaussTest ρ₀.im 1)).re < 0 :=
  effective_gaussian_dominance h₀ (by norm_num) hy (by norm_num) (by norm_num) hmax hsep hN
    (le_trans (effectiveThreshold_mono_B 20 (by norm_num) hB) (by linarith [threshold_example_ge]))
end AuditProbes
