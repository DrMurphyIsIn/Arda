...
| `SymmetricQuadFormEmitter` | the level-1 subset-indexed moment form `Φ = f₀·A² + 2f₁·A·X + f₂·X² + (f₁−f₂)·Q ≥ 0` **symbolically in n** (the `subsetForm_d1` object — one certificate covers all n), given moments obeying the rank-collapse identity | the exact completing-the-square + Cauchy–Schwarz congruence `Φ = f₀·(A+(f₁/f₀)X)² + cCS·(n·Q−X²)`: `field_simp; ring` identity + `div_nonneg`/`positivity` + `linarith` |
| `SymmetricQuadD2Emitter` | the **degree-2** subset-indexed moment form `Q₂ ≥ 0` **symbolically in n**, via the three-piece completing-the-square + Cauchy–Schwarz decomposition `Q₂ = (A+f₁s₁+f₂s₂)² + (n/4(n−1))·[centered CS] + μ₂·N₂` (`μ₂>0` for n>3); the level-≥1 scheme facts (`s₁²≤n·T₂`, `0≤N₂` — the independent level-2 norm) are supplied as hypotheses, matching the d=1 altitude | the assembly identity `Q₂ = …` symbolic in n by `field_simp; ring`, then `sq_nonneg` + `mul_nonneg` (`μ₂>0` × `N₂≥0`) + `div_nonneg` |
...
| `SeparableConvexExtremumEmitter` | separable-convex extremum on the fixed-sum box `{Σxᵢ=S, lᵢ≤xᵢ≤uᵢ}`, **both faces**: **min** = homogeneous point `n·φ(S/n) ≤ Σφ(xᵢ)` (Jensen) for convex `φ`, via the tangent line at `S/n`; **max** (`mode="max"`) = vertex `Σφ(xᵢ) ≤ (n−1)·φ(u)+φ(S−(n−1)u)` over the uniform box (convex even-degree-≤6 `φ`), parameterizing the proven `VertexLemmaFull` push-chain (`glemma_push_to_bound`/`vertex_bound`); refuses a non-convex `φ` | min: per-term `have … = Σcⱼ·bⱼ² := by ring; positivity` + `linarith`; max: `n−1` chained `nlinarith` push exchanges + `rw`/`norm_num`/`linarith` |
...
| `TightCapEnclosureEmitter` | the Brualdi–Goldwasser g-step **fixed-named-config** closure `(baseOf l)¹¹·prodBcap l /(W(5/3)¹¹) ≤ 1` (`W=64/621`, `Bcap μ=min(master_ub μ, min(glemma μ,1))`), two faces: **concrete** (all `μ` rational) decided by `norm_num` over the unfolded rational defs; **`symbolic1`** (one child `0<μ≤1/2`) by the reduce→monotone-fraction-cap→constant-power-cap chain `baseOf[μ]¹¹·glemma μ = W²(5/3)¹¹·((7+3μ)/(2(3+μ)))¹¹`, `(7+3μ)/(2(3+μ))≤17/14`, `W(17/14)¹¹≤1`, modeling the proven `single_child_le_one`; refuses a config whose exact LHS `> 1` (e.g. `μ=13/16`, peak ≈1.076). Scope: fixed-config `cert_jk` faces, NOT the general-arity g-lemma (`gV_le`) open core | concrete: `norm_num [baseOf, prodBcap, Bcap, master_ub, glemma, W, min_def, …]`; symbolic1: `field_simp; ring` (reduce) + `gcongr`/`nlinarith` (frac-cap) + `norm_num` (const-cap) + `div_le_one` assemble |
| `AffineParamEndpointEmitter` | an inequality whose gap `G(μ)=A+μ·B` is **affine in a parameter** μ holds on `[lo,hi]` **iff** it holds at the two endpoints (identity `(hi−lo)G(μ)=(hi−μ)G(lo)+(μ−lo)G(hi)`); ships the bV-shaped form `f x + μ·g x ≤ f c + μ·g c` (`f,g` opaque) that **collapses the BG SCLStep price-continuum `I=[456/3703,3/7]` to two rational checks**; refuses a gap with a μ² term (not affine) or a negative endpoint. Reusable for RH zero-free-region slabs affine along a line | `nlinarith [mul_nonneg (sub_nonneg.mpr hhi) hL, mul_nonneg (sub_nonneg.mpr hlo) hH]`; degenerate `lo=hi` via `le_antisymm` |
| `RecursionClosureEmitter` | the node-decouple **assembly** at a fixed price: tangent-majorant `nodeVal ≤ childBellSum + tangentBracket + μ·nodeY` (the proven `bell_node_tangent`/`bY_node`) + per-child ceiling ⟹ `nodeVal ≤ cherryVal`; all-cherry gives EQUALITY (composes with the `tight_cap_enclosure` tie); refuses an inconsistent ceiling or a price outside `I`. Honest scope: packages the assembly, NOT the log-tangent derivation nor the arity-unbounded all-cherry exchange (structural) | `linarith` (assembly) + `norm_num` (tie equality) |
| `CavityExchangeEmitter` | the Kelmans de-branch monotonicity `Aobj(t′)−Aobj(t)=P·FS·FQ·Φ ≥ 0`: a form `Φ` **affine in each of two marginals** is `≥0` on a box iff `≥0` at the four corners (barycentric identity), and each corner, domain-shifted to nonneg vars, is an **all-nonneg-coefficient Polya** polynomial; generalizes the fixed `R47R4Kelmans*Cert` corner certs; refuses a corner with a negative coefficient (Polya fails → needs SOS) or a negative corner value | corners: `positivity`; reduction: barycentric `ring` + `mul_nonneg` + `nlinarith` |
| `PerSizeDominanceSweepEmitter` | evolves `tight_cap_enclosure` into a **finite per-size sweep**: for fixed n, a list of configs each `(baseOf l)¹¹·prodBcap l/(W(5/3)¹¹) ≤ 1` (concrete `norm_num`) aggregated to `∀ l ∈ [configs], … ≤ 1`; refuses any enumerated config with LHS `>1`. Honest scope: closes each LISTED config at size n, NOT exhaustive over Balanced+Capped normal forms and NOT uniform-in-n (that is arm-rate unimodality) | per-config `norm_num`; aggregate `simp [List.forall_mem_cons]` + anonymous constructor |
| `CurvatureBoundaryEmitter` | a function with a **definite second-derivative sign** attains its extremum on `[a,b]` at the boundary — **concave** (`f''≤0`): `min(f a, f b) ≤ f x`; **convex** (`f''≥0`): `f x ≤ max(f a, f b)`; **affine** (`f''=0`): `m ≤ A+x·B ⟺ endpoints` (the `affine_param_endpoint` core in curvature framing). Ports AxiomMath/ZetaZeros `extremalG_const` (arXiv:2609.02882, `G''=0⟹affine⟹constant`), generalizes `affine_param_endpoint`, and covers the BG concave-corner reduction; refuses a wrong curvature sign (interval-aware `f''` check, incl. interior critical points) | abstract: `ConcaveOn.2` convex-combination / dual; concrete: `nlinarith [mul_nonneg (sub_nonneg.mpr _) (sub_nonneg.mpr _)]` on `(x−a)(b−x)≥0` |
| `TranscendentalEnclosureEmitter` | rational lower/upper bounds `L ≤ expr ≤ U` for a transcendental over a box, kernel-checked. **log face** (shipped): `log(1+x) ≤ x` (`Real.log_le_sub_one_of_pos`) + rational lower `L ≤ log(1+x)` on `[x0,x1]` via `Real.le_log_iff_exp_le` + `Real.exp_bound'` — discharges the BG per-cell `log(1+S/d)` term. **trig face** (`C₀ = 3/2−(1/√2)cot(1/√2)`, Montgomery–Taylor): deferred (needs √2 + cos/sin Taylor), refused at cert time so no non-green Lean is emitted; refuses any invalid rational bound | `Real.log_le_sub_one_of_pos`; `Real.le_log_iff_exp_le` + `Real.exp_bound'` (degree-3) |
| `LogCombinationEmitter` | a rational combination of logs is bounded: `Σᵢ cᵢ·log(rᵢ) ≤ q` by **folding** into a single `log(∏ rᵢ^{cᵢ})` — **tight at the tie** (no separate lower bound on any subtracted log, e.g. `F*`). **Three routes**: **monotone** (`q=0`: `∏ rᵢ^{cᵢ} ≤ 1` via `Real.log_le_log`), **tangent** (`log(∏) ≤ ∏−1` via `Real.log_le_sub_one_of_pos`, any-sign `q`, any `k` incl. negative `+F*`), and **tight** (degree-3 exp: `log X ≤ Q ⟺ X ≤ exp Q` via `Real.exp_bound'` — for cells where the degree-1 tangent overshoots). The F\*-folding companion to `transcendental_enclosure`; **dogfooded against BG** — regenerates `log74_le_4fstar` / `log54_sub_fstar_le` byte-for-byte, and the tight-route `log79_add_fstar` / `log119_sub_fstar` build GREEN against the live `R3Cert.BGSCLInduction`; refuses a combination that violates the bound | monotone: `Real.log_pow` + `Real.log_le_log`; tangent: `Real.log_le_sub_one_of_pos` + fold + `norm_num`; tight: `Real.log_le_iff_le_exp` + `Real.exp_bound'` + `X·U≤1` |
...
| `ExpThresholdEmitter` | **threshold-to-exponential domination** (SHAPES_AUDIT_48H section 2 rank 5; B N2, C 4.5): the exponential bound every "for lam beyond an explicit threshold the exponential wins" closure needs, by `1 + t ≤ e^t`. A **bundle** reads the nested-max threshold hypothesis `max 1 (max t₁ (max t₂ …)) ≤ lam` (the `eventual_threshold` witness, with the `max 1` guard folded in as a conjunct) and returns `1 ≤ lam ∧ C₁ ∧ …`, each step **linear** (`Q/(s·a·K) ≤ lam → Q ≤ K·exp(s·lam·a)`, or `<` when strict; crude, no sign on `Q`) or **log** (`log(max 1 (Q/K))/(s·a) ≤ lam → Q ≤ K·exp(s·lam·a)`; sharp, `Q ≤ 0` trivial). `Q`, `a`, `K` are each a symbol or an exact rational (then the threshold is a pinned number and the signs are re-decided in-kernel). Atoms: **product** `y·e^{−y} ≤ 1` (+ the commuted face), **inv** `e^{−x} ≤ 1/x` / `x0 ≤ x → e^{−x} ≤ 1/x0`, **shifted_rate** `(c₁y + c₀)e^{−ry} ≤ K e^{−r'y}` with the emitted `K = max(c₀, c₁/(r−r'), 0)`. **Dogfooded** on the rvm_bridge island: `E6Bridge7.lean:554-590` (threshold + `hexpη`/`hexpM` as one guarded bundle) and `E6Bridge14.lean:66-86` (`le_exp_of_log_le`, `effectiveThreshold`) regenerate line-for-line (`examples/rvm_bridge/lean/Probes/Dogfood_exp_threshold.lean`, design doc `docs/EMITTER_EXP_THRESHOLD_DESIGN_2026-09-22.md`). Refuses a rational `a ≤ 0` / `K ≤ 0`, a non-positive scale, a declared threshold not matching the exact recomputation, strict log, an inv bound below `1/x0`, `r' ≥ r`, a shifted-rate constant below the certified minimum, floats | linear: `div_le_iff₀` + `Real.add_one_le_exp` + `linarith` (+ `rwa [div_le_iff₀, mul_comm]` to clear `K`); log: `lt_max_of_lt_left one_pos` + `Real.exp_log` + `Real.exp_le_exp`; product: `Real.exp_neg` + `div_le_one` / `inv_mul_le_iff₀`; inv: `inv_anti₀` + `one_div_le_one_div_of_le`; shifted: `mul_le_mul_of_nonneg_*` + `Real.exp_add` |
| `EnclosureTreeEmitter` | **rational two-sided enclosures of an expression tree** over `{+, −, ×, /, ^, √, log, exp, π, arctan, rationals}` (SHAPES_AUDIT_48H section 2 rank 1; A N2/N3/N4, B C2, C 4.7, D 4): `lo ≤ E ≤ hi`, each side `<` exactly when the exact fold has slack or an OPEN endpoint (the strict Mathlib facts `π`, `log 2`). One Mathlib fact per atom (`Real.pi_gt_dN`/`pi_lt_dN` with the LEAST ladder rung that carries the claims; `log_two_gt_d9`; `abs_log_sub_add_sum_range_le` at the least fitting order plus the backwards `Real.log_mul` fold; `Real.exp_bound`; `Real.sq_sqrt` against two exact rational squares; `|arctan t| ≤ |t|` and `t/2 ≤ arctan t ≤ t` on `[0,1]`), an exact interval fold for the operations; LINEAR nodes are seen through by the parent's `linarith`, one theorem per non-linear / named / claimed node, shared subtrees ONCE across the family. Faces: the `π` **rate corollary** `n ≤ cap → (n + 1 : ℝ) ≤ E` and the **log/sqrt** face `log (y + a) ≤ c √y` on `y ≥ y₀`. **Dogfooded** on the li_positivity island: `LiLadderHeight`/`LiLadderSharp`'s `pi_gt_d4`/`pi_gt_d6` rate steps (the ladder search lands on d4/d6 by itself, kernel cross-checks feed the emitted rate lemmas to the hand theorems) and `LeakageDictionary.lean:276-359`'s nine bracket lemmas with their original claims (`examples/li_positivity/lean/Probes/Dogfood_enclosure_tree.lean`; quasicrystal-side cross-checks in `examples/quasicrystal/lean/Probes/Dogfood_enclosure_tree.lean`; design doc `docs/EMITTER_ENCLOSURE_TREE_DESIGN_2026-09-22.md`). Refuses a claim the fold does not imply at ANY node (negative control), strict without slack, a radicand not `≥ 0`, a denominator interval containing 0 or negative, `log r` with `r ≤ 0`/`r = 1`, `|1 − r| ≥ 1` without a factorisation, a fold not multiplying to `r`, orders outside `1..64`, `exp` at `|x| > 1`, a negative base under a power, an off-ladder `π` rung, an unreached rate cap, rational-only trees, floats | `π`: `linarith [Real.pi_gt_dN, Real.pi_lt_dN]`; √: `Real.sq_sqrt` + `nlinarith [h2, hn, …]`; log: `abs_log_sub_add_sum_range_le` + `generalize` + `norm_num [Finset.sum_range_succ]` + `linarith`, fold `rw [← Real.log_mul …]`; exp: `Real.exp_bound` + `norm_num [Nat.factorial]`; ×: four McCormick `mul_nonneg` facts + `linarith`; /: `le_div_iff₀`/`div_le_iff₀` + `linarith`; ^: `pow_le_pow_left₀`; rate: `exact_mod_cast` + `linarith` |
| `PreorderingMultiplierEmitter` | **positivity on a semialgebraic set with POLYNOMIAL generators, by a positive multiplier** (SHAPES_AUDIT_48H section 2 rank 3; D 3.1, B C3, C 5.3): `0 ≤ p` on `{g₁ ≥ 0, …, gₘ ≥ 0}` from the exact constant-coefficient preordering identity `M·p = Σ c_α ∏ gᵢ^{αᵢ}` with every `c_α ≥ 0` and `M = κ·g_j^e` (`κ > 0`, `e ≤ 3`) or a constant, the combination `handelman` (linear generators, no multiplier), `polya_zeros` (simplex only) and `rational_sos` (no generator hypotheses) each miss. Generators are tagged `hyp` (a theorem hypothesis `lhs ≤ rhs` whose content must be exactly `gᵢ`) or `structural` (closed by `positivity`, checked conservatively). The zero set of a non-constant `M` must be ONE point (`g_j` an exact positive-weight sum of squared coordinate offsets, derivable with `locus="auto"`), where `p ≥ 0` is exact. Supply the certificate, or return `terms=None` and an exact rational phase-1 simplex FINDS it (outer loop over multiplier candidates, `"auto"` = `1`, then `g_j^{1..3}` for every single-point generator; inner loop over the product degree). Optional thin complex face `0 ≤ (<target>).re` (the `complex_re_im_split` step, unfold list from the island). **Dogfooded** on the li_positivity island: `LiBoxRungs.lean` `re_Q1..re_Q5_nonneg` regenerated (the three hand certificates with multipliers `1`, `14 s`, `s²` verbatim, plus LP-found `Q₄`/`Q₅`), drop-in for the island's termwise dispatch, and `Q₆` REFUSED as OBSTRUCTED_AND_LOCATED at the exact disk point `(4/5, −2/5)` (`Re Q₆ = −27772/15625`), the refutation itself kernel-checked (`examples/li_positivity/lean/Probes/Dogfood_preordering_multiplier.lean`, design doc `docs/EMITTER_PREORDERING_MULTIPLIER_DESIGN_2026-09-22.md`). Refuses the LP infeasible up to the cap (with a located negative witness when the exact grid scan finds one; otherwise explicitly NOT a verdict), any `c_α < 0`, an identity that does not reconstruct, a `hyp` generator that is not literally a hypothesis, a `structural` one `positivity` cannot close, a multiplier with an uncertified zero locus or not in the cone, degree/term/LP/scan caps, floats, name collisions | `obtain ⟨g, hg0, hge⟩` per used generator (`linarith` / `positivity`); `generalize hP : p = P`; `have key : M * P = Σ … := by rw [← hP, …]; ring`; `hcert` by `rw [key]; positivity`; `rcases eq_or_lt_of_le` → locus branch (`nlinarith only [sq_nonneg …]` + `norm_num`) or `(mul_nonneg_iff_of_pos_left (by positivity)).mp hcert`; `M = 1` collapses to `rw [key]; positivity`; the refusal's refutation: `intro h; have hw := h a b (by norm_num); norm_num at hw` |

| `ComplexReImSplitEmitter` | the **real / imaginary-part split of a complex polynomial expression** (SHAPES_AUDIT_48H section 2 rank 4; B N3, B D7, C 2.10, D 2.2 -- "the smallest and most frequent shape in the cluster"): for `p` polynomial in a complex `z`, real atoms `(c : ℝ)` and natural-number atoms `(m : ℕ)` cast into `ℂ`, `Complex.I` and rational constants, the faces `(p : ℂ).re = P_re`, `(p : ℂ).im = P_im`, `‖p‖ ^ 2 = P_re^2 + P_im^2`, `‖Complex.exp p‖ = Real.exp P_re`, and the B D7 **cast face** of a real-valued `p` (no `z`, no `I`): `(p : ℂ) = ((P : ℝ) : ℂ)` and its `.re` through that identity.  The claimed real polynomial IS the statement; sympy's split at `z = x + i y` is re-verified by a symbolic identity, by `as_real_imag` and by an independent exact `Fraction` evaluator, the claim must be ring-equal to it, and `assert_certificate_sensitive` is wired.  **Dogfooded** on the rvm_bridge island (`examples/rvm_bridge/lean/Probes/Dogfood_complex_re_im_split.lean`, 15 theorems, design doc `docs/EMITTER_COMPLEX_RE_IM_SPLIT_DESIGN_2026-09-22.md`): `E6Bridge28.re_pow_two..five` with kernel TIE GATES `:= RvMBridge28.re_pow_N`; the `hre`/`hsq`/`hw2re`/`hw2im`/`hEim` `have`s of `E6Bridge7.norm_gaussTest`/`re_gaussTest` plus the `Complex.norm_exp` step, with CONSUMER GATES (each hand lemma pinned by `:= RvMBridge7.<lemma>` and RE-PROVED from the emitted splits, the residual `neg_mul` + three `ring` reshapings declared); and the five B D7 cast `have`s of `E6Bridge5`/`7`/`11`/`14`, each with an INSTANTIATION GATE (the hand statement, verbatim, closed by the emitted theorem at the island's atoms).  Refuses a non-polynomial `p` (division, transcendental, `conj`/`re`/`im`/`Abs`, a symbolic exponent; the declared-denominator face is a follow-on), a claim not ring-equal to the split (the forge case / negative control), the cast face on a `p` with `z` or `I`, undeclared, non-real or mis-declared natural atoms, floats, degree above the cap (10), a degenerate bare variable / atom / constant, a `tie_to` that is not a Lean identifier, a reserved binder name (a keyword, `Complex` / `Real`).  Finite polynomial bookkeeping, NOT RH | the frozen skeleton of the hand proofs: `simp only [Complex.add_re, Complex.add_im, Complex.sub_re, Complex.sub_im, Complex.mul_re, Complex.mul_im, Complex.neg_re, Complex.neg_im, Complex.ofReal_re, Complex.ofReal_im, Complex.natCast_re, Complex.natCast_im, Complex.I_re, Complex.I_im, Complex.re_ofNat, Complex.im_ofNat, Complex.one_re, Complex.one_im, Complex.zero_re, Complex.zero_im, pow_succ, pow_zero, one_mul]` (restricted per instance to the lemmas the projection propagation uses) then `all_goals ring`; `norm_sq` after `rw [Complex.sq_norm, Complex.normSq_apply]`; `norm_exp` via `have hre := ...; rw [Complex.norm_exp, hre]`; cast face `push_cast` then `all_goals ring` (then `rw [hcast, Complex.ofReal_re]`); a statement-match `example` after every theorem |

| `ZeroSumMajorantEmitter` | **zero-sum majorant** (SHAPES_AUDIT_48H section 2 rank 2; C 3.1 + B N5): a family `f : ℂ → ℂ` supported on the nontrivial zeros is summable through a finite ordinate window `\|Im ρ − a\| < h` plus the local-count tail `m(ρ)·C/(1 + \|γ_ρ\|²)` (the island atom `RvMBridgeXi.zeroBoundAt`). Per instance the kernel content is ONE strip inequality `N/D ≤ C/(1 + \|γ_ρ\|²)` on `h ≤ \|Im ρ − a\|` (`D` ∈ {`normSq ρ`, `(Im ρ − a)²`, `1 + (Im ρ − a)²`}, `h` ∈ {0,1,2}, centre a rational or a polynomial in declared real parameters such as `Im s`), cleared to an EXACT nonnegative Bernstein/Polya combination of `Re ρ`, `1 − Re ρ`, `(Im ρ − a)² − h²`, parameter squares and declared squares (found by the audit's `w² → h² + t` shift + odd-part removal + degree elevation; exact sympy, no LP). Faces `_strip`, `_le` (pointwise `‖f ρ‖ ≤ zeroBoundAt a h ([K·]C) b ρ` from the named hypotheses `hzero`/`hwin`/`hshape`, optional prefactor `K ≥ 0`) and `_summable`; mode **tail_envelope**: `‖∑'_S f‖ ≤ E·∑' w` (rational or symbolic `E`) and the rate companion `e^{2λφ} ≤ e^{2(λ−1)P}e^{2φ} ∧ e^{2(λ−1)P} ≤ 1` at a rational `P < 0`. **Dogfooded** on the rvm_bridge island: E6Bridge19 `zbound`, E6Bridge18 `polBound`, E6Bridge15 `liBound` and E6Bridge12 `tail_bound_window` regenerate under new names with kernel cross-checks against the originals (`examples/rvm_bridge/lean/Probes/Dogfood_zero_sum_majorant.lean`, design doc `docs/EMITTER_ZERO_SUM_MAJORANT_DESIGN_2026-09-22.md`). Refuses `C < 0` / a non-positivity `C` or `N`, a failed Polya check (FALSE with a located rational witness -- the `h = 0`, `1/\|ρ\|²` phantom -- or OBSTRUCTED), `h` outside {0,1,2}, a non-ordinate window, a conditional support fact, `ordinate_sq` with `h = 0`, a term list that is not the residual, `P ≥ 0` / `E < 0`, floats, colliding names | strip: `hz.2.1`/`hz.2.2` + `pow_le_pow_left₀` + `sq_abs` + `div_le_div_iff₀`, the certificate as `have key : C*D − N*G = Σ c_α·P_α := by ring`, one `mul_nonneg` term per summand, `linarith only [key, t…]`; `_le`: `RvMBridgeXi.norm_le_zeroBoundAt` + `mul_le_mul_of_nonneg_left`; `_summable`: `Summable.of_norm_bounded` + `RvMBridgeXi.summable_zeroBoundAt`; envelope: `norm_tsum_le_tsum_norm` + `tsum_le_tsum` + `tsum_subtype_le` + `tsum_mul_left`; rate: `Real.exp_add` + `Real.exp_le_exp` + `Real.exp_le_one_iff` |
...
| `RobinGrowthEmitter` | **RH Face 2 (temperedness).** A rung of Robin's theorem (RH ⟺ σ(n) < e^γ·n·log log n for all n > 5040): the exact divisor sum σ(n) and a rigorous rational lower bound `Llo` (flint/Arb) on the transcendental RHS `R = e^γ·n·log log n`, carried as hypothesis `hR : Llo ≤ R`, give `(σ(n):ℝ) < R`; ships `robin_neg_refutes` (a certified upper bound below σ(n) ⟹ ¬RH). Refuses `n ≤ 5040` or `Llo ≤ σ(n)`. A finite rung, NOT RH | `lt_of_lt_of_le (by norm_num) hR` — the exact `(σ:ℝ) < Llo` by `norm_num`, chained through the Arb enclosure hypothesis |
| `BaezDuarteEmitter` | **RH Face 6 (spectral / approximation).** A rung of the Nyman–Beurling–Báez-Duarte criterion (RH ⟺ d²_N → 0, `d²_N = inf_c ‖1 − Σ c_k·{(1/k)/x}‖²_{L²(0,1)}`): an explicit rational coefficient vector gives a form value `Q(c) ≥ d²_N` whose Gram integrals are rigorously enclosed (exact piecewise antiderivative on `[δ,1]` + flint/Arb `ln`-values + a `[0,δ]` tail bound), yielding a rational `Uexact`; `dN2 ≤ Uexact` is the hypothesis `hval`, giving `dN2 ≤ U` (`U ≥ Uexact` a readable rounded-up cap). Ships `bd_neg_refutes`. Bounds DECREASE with N (RH-consistent). Refuses `U < Uexact` or `k < 2`. A finite rung, NOT RH | `le_trans hval (by norm_num)` — the Arb-enclosed form value carried as hypothesis, `Uexact ≤ U` closed by `norm_num` |
