...
| `LogCombinationEmitter` | a rational combination of logs is bounded: `Σᵢ cᵢ·log(rᵢ) ≤ q` by **folding** into a single `log(∏ rᵢ^{cᵢ})` — **tight at the tie** (no separate lower bound on any subtracted log, e.g. `F*`). Two routes: **monotone** (`q=0`: reduce to `∏ rᵢ^{cᵢ} ≤ 1` via `norm_num` + `Real.log_le_log`) and **tangent** (`q>0`: `log(∏) ≤ ∏−1` via `Real.log_le_sub_one_of_pos` + `norm_num`). The F\*-folding companion to `transcendental_enclosure`; **dogfooded against BG** — regenerates `log74_le_4fstar` / `log54_sub_fstar_le` in `BGSCLSubaction.lean` byte-for-byte (modulo ascriptions); refuses a combination that violates the bound | monotone: `Real.log_pow` + `Real.log_le_log` + `linarith`; tangent: `Real.log_le_sub_one_of_pos` + `Real.log_mul`/`log_inv`/`log_pow` fold + `norm_num` |
