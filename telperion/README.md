...
| `PSDFormEmitter` | `0 ≤ xᵀMx` for an explicit rational symmetric **positive-semidefinite** matrix `M` (definite or singular), via the exact rational completing-the-square congruence `xᵀMx = Σ cᵢ·(…)²` (`cᵢ > 0`, cvxpy-free — the moment-matrix / Gram-bridge PSD primitive); refuses an indefinite matrix | `have xᵀMx = Σ Dᵢ·(Lᵀx)ᵢ² := by ring`, then `positivity` |
