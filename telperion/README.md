...
| `PSDFormEmitter` | `0 ≤ xᵀMx` for an explicit rational symmetric **positive-definite** matrix `M`, via the exact rational LDLᵀ congruence `xᵀMx = Σ Dᵢ·(Lᵀx)ᵢ²` (`Dᵢ > 0`, cvxpy-free — the moment-matrix / Gram-bridge PSD primitive); refuses a non-PD matrix | `have xᵀMx = Σ Dᵢ·(Lᵀx)ᵢ² := by ring`, then `positivity` |
