...
| `TangentSumEmitter` | a symmetric-sum (combinatorial) inequality `B ≤ Σf(xᵢ)` for a convex quadratic `f` with `Σxᵢ = S`, via the tangent line at `a = S/n` — the surplus is the exact SOS `c₂·Σ(xᵢ−a)²`; refuses a non-convex `f` | `nlinarith [sq_nonneg (xᵢ − a), …, hsum]` |
