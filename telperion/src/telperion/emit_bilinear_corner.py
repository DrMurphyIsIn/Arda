...
class BilinearCornerBoxEmitter(Emitter):
    ...
    def _instance_text(self, cert: BilinearCornerCertificate, lean_name: str) -> str:
        ...
        return (
            ...
            f"  have h00 : (0:ℝ) ≤ {A} + {B} * {s0} + {C} * {t0} + {E} * ({s0} * {t0}) := by\n"
            f"    have : {A} + {B} * {s0} + {C} * {t0} + {E} * ({s0} * {t0}) = ({c00} : ℝ) := by norm_num\n"
            f"    rw [this]; exact hc00\n"
            f"  have h01 : (0:ℝ) ≤ {A} + {B} * {s0} + {C} * {t1} + {E} * ({s0} * {t1}) := by\n"
            f"    have : {A} + {B} * {s0} + {C} * {t1} + {E} * ({s0} * {t1}) = ({c01} : ℝ) := by norm_num\n"
            f"    rw [this]; exact hc01\n"
            f"  have h10 : (0:ℝ) ≤ {A} + {B} * {s1} + {C} * {t0} + {E} * ({s1} * {t0}) := by\n"
            f"    have : {A} + {B} * {s1} + {C} * {t0} + {E} * ({s1} * {t0}) = ({c10} : ℝ) := by norm_num\n"
            f"    rw [this]; exact hc10\n"
            f"  have h11 : (0:ℝ) ≤ {A} + {B} * {s1} + {C} * {t1} + {E} * ({s1} * {t1}) := by\n"
            f"    have : {A} + {B} * {s1} + {C} * {t1} + {E} * ({s1} * {t1}) = ({c11} : ℝ) := by norm_num\n"
