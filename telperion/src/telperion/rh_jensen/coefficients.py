...
def enclose_coeff_box(n: int, d: int, prec_bits: int) -> list[tuple[Fraction, Fraction]]:
    ...
    Computes the xi power series once to the highest needed index (2*(n+d)) and
    slices out the even coefficients.
    """
    if n < 0 or d < 0:
        raise ValueError("n and d must be >= 0")
    max_index = 2 * (n + d)
    coeffs = _xi_series_coeffs(max_index, prec_bits)
    return [_arb_ball_to_fractions(coeffs[2 * k].real) for k in range(n, n + d + 1)]
