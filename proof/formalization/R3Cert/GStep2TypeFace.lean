...
/-- c-antitone cleared (master_core shape): `64·U^11 ≤ 621·V^11`, `a≥1`. -/
lemma cleared_c (a c : ℕ) (ha : 1 ≤ a) :
  ...
  have hstep : U ≤ (58 / 51) * V := by
    ...
    nlinarith [mul_nonneg (by linarith : (0:ℚ) ≤ (a:ℚ) - 1) (by positivity : (0:ℚ) ≤ 63*(a:ℚ)+92),
      mul_nonneg hc' ha', sq_nonneg (c:ℚ), hc', ha']
  calc (64 : ℚ) * U ^ 11
      ≤ 64 * ((58 / 51) * V) ^ 11 :=
...
/-- Per-step c-ratio: `base2(a,c+1)^11 · W ≤ base2(a,c)^11`, `a≥1`. -/
lemma key_c (a c : ℕ) (ha : 1 ≤ a) :
    (base2 a (c + 1)) ^ 11 * (64 / 621) ≤ (base2 a c) ^ 11 := by
  unfold base2
  rw [div_pow, div_pow, div_mul_div_comm,
    div_le_div_iff₀ (by positivity) (by positivity)]
  have lhs_eq : (9 * (a : ℚ) + 12 * (c + 1) + 8) ^ 11 * 64 * (6 * ((a : ℚ) + c + 1)) ^ 11
      = 64 * ((9 * (a : ℚ) + 12 * c + 20) * (6 * ((a : ℚ) + c + 1))) ^ 11 := by
    rw [mul_pow]; push_cast; ring
  have rhs_eq : (9 * (a : ℚ) + 12 * c + 8) ^ 11 * ((6 * ((a : ℚ) + (c + 1) + 1)) ^ 11 * 621)
      = 621 * ((9 * (a : ℚ) + 12 * c + 8) * (6 * ((a : ℚ) + c + 2))) ^ 11 := by
    rw [mul_pow]; push_cast; ring
  rw [lhs_eq, rhs_eq]
...
lemma cstep (a c : ℕ) (ha : 1 ≤ a) : GS2 a (c + 1) ≤ GS2 a c := by
  have hbw : (0 : ℚ) ≤ Bhalf ^ a * (64 / 621 : ℚ) ^ c := by positivity
  have hfp1 : GS2 a (c + 1)
      = (base2 a (c + 1)) ^ 11 * (64 / 621) * (Bhalf ^ a * (64 / 621) ^ c) := by
    unfold GS2 W; rw [pow_succ]; ring
  rw [hfp1]; unfold GS2 W
...
/-- a-antitone cleared: `Bcap(½)·Ua^11 ≤ Va^11`, `a≥1`. -/
...
lemma astep (a : ℕ) (ha : 1 ≤ a) : GS2 (a + 1) 0 ≤ GS2 a 0 := by
  ...
  have hfp1 : GS2 (a + 1) 0
    ...
    unfold GS2 base2; rw [pow_succ, pow_zero]; push_cast; ring
  have hfp0 : GS2 a 0 = ((9 * (a : ℚ) + 8) / (6 * ((a : ℚ) + 1))) ^ 11 * Bhalf ^ a := by
    unfold GS2 base2; rw [pow_zero]; push_cast; ring
  rw [hfp1, hfp0]
  have hkey : ((9 * (a : ℚ) + 17) / (6 * ((a : ℚ) + 2))) ^ 11 * Bhalf
    ...
    have le : (9 * (a : ℚ) + 17) ^ 11 * Bhalf * (6 * ((a : ℚ) + 1)) ^ 11
        = Bhalf * ((9 * (a : ℚ) + 17) * (6 * ((a : ℚ) + 1))) ^ 11 := by rw [mul_pow]; ring
    have re : (9 * (a : ℚ) + 8) ^ 11 * (6 * ((a : ℚ) + 2)) ^ 11
        = ((9 * (a : ℚ) + 8) * (6 * ((a : ℚ) + 2))) ^ 11 := by rw [mul_pow]
    rw [le, re]
