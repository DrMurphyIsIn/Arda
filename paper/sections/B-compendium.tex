...
This appendix is a \emph{dated snapshot}---2026-09-09, at 135 emitter
...
\texttt{comparability\_envelope} & comparability / Lipschitz envelope atoms \\
\texttt{discrete\_moment} & discrete-sum moment atoms \\
\texttt{poly\_exp\_absorption} & inverse-power prefactors absorbed into halved exponentials \\
\texttt{graded\_convolution} & exact identity calculus for graded convolutions \\
\texttt{power\_tower} & cubic self-composition closures $C_q \le (9L)^{3^q}$ \\
\texttt{low\_order\_tail} & explicit low grades plus doubled geometric tails \\
\texttt{eventual\_threshold} & eventual thresholds with generated max-of-ratio witnesses \\
\addlinespace
...
\texttt{regular\_word} & forbidden-factor word invariants (first discrete/word axis) \\
\texttt{partition\_composition} & Fa\`a di Bruno factorial-square partition bounds (verbatim-ported core; see \texttt{NOTICE}) \\
\addlinespace
...
\addlinespace
\multicolumn{2}{@{}l}{\emph{Extremal-combinatorics and proof-complexity emitters: reported in the companion Telperion paper~\cite{telperion-companion}.}} \\
\addlinespace
...
passes; attributions in \texttt{NOTICE}); a Palomar-registry poller,
arXiv/GitHub/Zulip source monitors, frontier-formalization repository adapters,
and a Comparator-challenge ingest that surface new formalizations as emitter
candidates and port leads.
