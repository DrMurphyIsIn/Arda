...
classification program itself we leave exactly where Dyson left it---noting, for
honesty's sake, that the one rigorously classified case (integer-valued Fourier
quasicrystals, via Lee--Yang polynomials \cite{kurasovsarnak2020}) is a class to
which the zeta measure provably does \emph{not} belong, so the analogy's ceiling
is itself a theorem.
...
checkers is the basis of our strongest confidence claim. In the results section, the
underlying mathematics is due to others and cited there: de la Vallée-Poussin for
the classical zero-free region \cite{dlvp1896}.
