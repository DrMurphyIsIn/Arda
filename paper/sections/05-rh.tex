...
ladder supplies a quarter-million certified zeros to diffract. Putting the two
together produces the picture this paper is named for
...
against a noise band of $1.3\times10^{-2}$). Pure point spectrum on the
...
instance---a kernel-checked enclosure of $F_{100}(u^*)$ at the rational
frequency $u^* = 693147/10^6 \approx \log 2$ over the twenty-nine zeros below
height $100$, complete with the count-anchored argument that the twenty-nine
witnessed zeros are \emph{all} the zeros---is in progress on the same
certificate pipeline. When it scales, theorems of the form
$|F_T(m\log p) - A| \le \varepsilon$ will constitute, to our knowledge, the
