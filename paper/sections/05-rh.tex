...
doubled the height to $200$; the current milestone, \texttt{AllZeros\_h1000},
tiles \emph{nineteen} bands to reach $T = 1000$: all $649$ nontrivial zeros with
$0 < \operatorname{Im}\rho \le 1000$ lie on the critical line. The tiling is
\emph{density-adaptive}---one band of height $100$ at the bottom (winding
$N = 29$), then eighteen bands of height $50$, the band height halving where the
zero density (and hence the proof cost per band) grows. The nineteen boundary
windings sum to $649$---the full classical count to that height---though as
before the count is derived from the contours, not assumed. The tiling refinement
also buys a cross-check for free: the eighteen half-bands were computed
independently of the original height-$100$ contours, and each adjacent pair of
half-band windings reproduces its parent band's winding to the integer. (An
...
$1000$, not all of $\mathbb{C}$; Turing's method verifies RH in a region, it does
not prove RH. And it is \emph{conditional on certified numeric input}: the
per-band boundary winding integers, the $649$ exhibited on-line zeros, and the
...
Turing verification up to height $1000$, and it exhibits a third distinct
...
rectangular Guinand--Weil identity at finite height. Weighted zero-sums equal
prime sums plus three explicit boundary integrals---zeros diffracting off primes,
in the kernel.
