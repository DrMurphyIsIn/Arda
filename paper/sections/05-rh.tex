...
This section presents the paper's main results---first, in bold, so there is no
chance of misreading: \textbf{we do not claim the Riemann Hypothesis.} What we have is a connected body of genuine,
...
they \emph{are} (a complete finite Turing verification to height \ladderHeight{}), and
...
\begin{enumerate}
  ...
  \item an elementary, Hadamard-free zero-free region
    $\operatorname{Re} s > 1 - c/|t|^5$ (\texttt{riemannZeta\_zero\_free\allowbreak\_poly});
  \item a sharper region $\operatorname{Re} s > 1 - c/\!\left(|t|^4 (1 + \log 2|t|)\right)$
    (\texttt{riemannZeta\_zero\_free\allowbreak\_polylog}).
\end{enumerate}

To our knowledge, the last two are the first unconditional, Hadamard-free
zero-free regions to be machine-checked in Lean.

\subsection{One engine, four certificate shapes}

The results that follow are not unrelated: they are four readings of one machine.
Every certificate below is an instance of a single weighted argument
principle---for analytic $f$ and holomorphic weight $g$, the boundary winding of
$g$ against $f$ equals $2\pi i \sum_{\rho} \operatorname{mult}(\rho)\, g(\rho)$---and
the results exhibit it in four distinct \emph{certificate shapes}: positivity,
real-rootedness, argument-principle winding, and the explicit-formula/diffraction
identity. That one finite engine reaches all four on the same object is the
methodological point, and the shape-diversity is deliberate evidence of the
method's range. We stress at the outset what this unification does \emph{not}
claim: the finite instances are deliberately not stitched into any assertion that
the count $N(T)$ determines the Li coefficients $\lambda_n$ (\cref{sub:bridge}).
Each result remains finite, each falls well short of RH, and
$\texttt{conjecture1\_proved} = \texttt{False}$ throughout.
...
\texttt{all\_nontrivial\_zeros\_up\_to\allowbreak\_height\_on\_line\_tiled}) compose
...
(\texttt{all\_nontrivial\_zeros\_in\allowbreak\_segment\_on\_line}) compose with an
...
height. Its first exercise was an early milestone, \texttt{AllZeros\_h4000}:
...
knowledge the first kernel-checked Turing verification at any height, and a
height at the machine-computed frontier of the mid-1960s (Lehman's
...
in the zero count and says nothing about where the zeros lie. Feeding this back
into the counting formula pins the count to its main term:
$|N(T) - 1 - \theta(T)/\pi| \le \log\!\big((4T{+}19)/(2-\pi^2/6)\big)/\log(7/6) + 2$
(\texttt{nt\_effective\_bound}), so $N(T)$ sits within an explicit $O(\log T)$ of
$1 + \theta(T)/\pi$---an effective Riemann--von Mangoldt count, the classical
error term made explicit. It too counts zeros, not places them. Second, one mid-course
...
ladder supplies a quarter-million certified zeros to diffract. The oldest
...
the random-phase scale $\sqrt{N/2} \approx 341$ everywhere \emph{except} at the
...
significant figures (maximum deviation $2\times10^{-4}$ in normalized units,
against a noise band of $1.3\times10^{-2}$). The resonances are sharp in the
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/dyson_diffraction_T160000}
...
bracket width, and an interval fold over the twenty-nine terms. One
formulation gap is carried honestly: the theorem sums over the twenty-nine
\emph{witnessed} zeros (exactly what the certified sign chain exhibits);
promoting it to the abstract zero set via the exactly-twenty-nine count of
\texttt{AllZeros\_h100} is a pigeonhole step in progress. As the pipeline
...
\subsection{The archimedean growth, closed to a linear window; and the diffraction floor}

Two developments have since moved the frontier that the previous subsection drew.
First, the growth residual is now substantially \emph{closed}---by yet another
reroute. The polygamma closed form above is exact but combinatorially opaque
(sums over ordered set partitions); composing the same three kernel stones
differently---the M\"obius chain rule, the reduction to $\psi(s/2)$, and the
Gauss--Weierstrass digamma series, with every denominator cleared through the
M\"obius map---turns the \emph{whole} archimedean factor into one elementary
series of rational functions (\texttt{logDeriv\_phi\_Gamma}\(\mathbb{R}\)\texttt{\_eq\_elem}),
whose $n$-th Taylor coefficients are explicit. The kernel then holds the exact
elementary formula
\[
  \texttt{taylorCoeff}(\Gamma_{\mathbb R})_n
    = -\frac{\gamma + \log\pi}{2}(n+1) - 1
      + \sum_{j\ge 0}\Bigl[\frac{n+1}{2(j+1)} - 1
        + \Bigl(\frac{2j+2}{2j+3}\Bigr)^{n+1}\Bigr]
\]
(\texttt{taylorCoeff\_Gamma}\(\mathbb{R}\)\texttt{\_elem}; the term-by-term coefficient
extraction through the infinite sum is done by iterating the locally-uniform-limit
derivative theorem over partial sums---no per-order bounds, one uniform
$2/(j+1)^2$ envelope), and, from it, the unconditional growth window
\[
  \Bigl|\,\mathrm{Re}\,\texttt{taylorCoeff}(\Gamma_{\mathbb R})_n
    - \tfrac{n}{2}\log n\,\Bigr| \;\le\; 8n \qquad (n \ge 2)
\]
(\texttt{taylorCoeff\_Gamma}\(\mathbb{R}\)\texttt{\_re\_growth}), by Bernoulli and
second-Bonferroni bounds on each summand, a split of the sum at $j = n$, and
Mathlib's harmonic--logarithm inequalities. The sentence ``we do not have its
asymptotic'' above is thereby superseded in its main clause: the $(n/2)\log n$
trend of the archimedean summand is a kernel theorem, unconditionally, with an
explicit linear window. What remains unformalized is the sharper classical form
$\tfrac{n}{2}(\log n + \gamma - 1 - \log 2\pi) + o(n)$---the constant of the
linear term---and, as ever, everything about the \emph{companion} summand, whose
temperedness is where the Riemann Hypothesis itself lives (Voros's dichotomy);
nothing here touches it.

Second, the companion has acquired its own finite certificate family. Through the
relocation $\mathrm{RH} \leftrightarrow \forall n,\ -(1 +
\mathrm{Re}\,\texttt{taylorCoeff}(\Gamma_{\mathbb R})_n) \le
\mathrm{Re}\,\texttt{taylorCoeff}(H_\zeta)_n$ the explicit archimedean data
becomes a \emph{floor}, and a Telperion emitter (\texttt{bragg\_floor}) certifies,
per order, that the truncated von Mangoldt (Bragg) amplitude at a fixed base
point, net of a certified tail, clears it---fifteen kernel-checked rational
inequalities, with the Arb enclosures the same documented trust seam as the Li
ladder, and a falsifiability atom
(\texttt{bragg\_below\_floor\_refutes\_rh}) whose passage to the companion runs
through the \emph{conditional} exhaustion identity and carries it as an explicit,
undischarged hypothesis. The emitter \emph{refuses} the orders its fixed-base
amplitude cannot clear rather than fake the RH-hard comb expansion---the honest
shape of a certificate family that could have falsified and did not. As
everywhere: \texttt{conjecture1\_proved = False}.
...
One instrument stood between the program and a zero-hypothesis band---a
kernel value for $\theta(t)$ itself---and it has now been built. The corpus
route (quadrature of the enclosed $\log\Gamma_{\mathbb{R}}$-derivative) was
genuinely blocked: it requires a digamma series representation and polygamma
bounds that exist nowhere in Mathlib, and we declined to manufacture them
mid-campaign. But a back door stood open, and it is three centuries old:
Euler's limit formula (already formalized in Mathlib as the
$\Gamma$-sequence) gives $\arg\Gamma(z)$ at finite stage $n$ as
$y\log n - \sum_{k\le n} \arctan\!\big(y/(x+k)\big)$---arctangents at
rational arguments, exactly telescoping harmonic sums, and explicit
$O(1/n)$ rates, every ingredient already in the toolkit. Through it,
the phase $\theta(14)$ and $\theta(15)$ were boxed in the kernel (the
convergence and its rate discharged as theorems, not hypotheses; the
$\arctan$ near its awkward argument $u\approx1$ handled by the addition
identity $\arctan u = \tfrac\pi4 + \arctan\tfrac{u-1}{1+u}$), the sign
quantity $\cos\theta\cdot\operatorname{Re}\zeta -
\sin\theta\cdot\operatorname{Im}\zeta$---the Hardy $Z$-function---was
exhibited negative at $14$ and positive at $15$ from kernel-computed
boxes on both factors, and the intermediate value theorem closed it:
\[
  \texttt{first\_zero\_kernel} \;:\;
  \exists\, r,\ 14 < r < 15 \ \wedge\
  \Lambda\!\left(\tfrac12 + r i\right) = 0,
\]
axioms $\{$\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}$\}$,
no \texttt{sorryAx}, no \texttt{ofReduceBool}, and---the point---\emph{no
arguments and no numeric hypotheses whatsoever}. To our knowledge this is
the first nontrivial zero of the Riemann zeta function whose existence has
been verified by \emph{pure computation inside a proof-assistant kernel}:
not a verified \emph{account} of a computation, but the computation itself,
carried out where nothing unverified can reach it. It is the natural
endpoint of the trust programme this paper describes, and its method is
height-agnostic---the second zero, and the thousandth, cost only
certificates. As everywhere, the honesty line is exact:
\texttt{conjecture1\_proved = False}. This is \emph{one} certified zero, at
$\gamma_1 \approx 14.1347$; it is not, and does not approach, a proof of the
Riemann Hypothesis. The reflection program changes what a finite
verification \emph{is}, not what it proves.
...
The methodological point is that the same untrusted-generator/trusted-kernel
discipline makes it easy to state exactly how strong these results are and are
not. The same discipline applies well beyond the zeta function---to a still-open
extremal-combinatorics problem and a proof-complexity lower bound (mathematics
due to others)---reported separately~\cite{telperion-companion}.
