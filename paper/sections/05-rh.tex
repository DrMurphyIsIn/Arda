...
What remains is then a single, named obstruction---and naming it precisely is
more honest than calling it ``hard analysis.'' The Bombieri--Lagarias main term
$\lambda_n \sim (n/2)\log n$, the one place the zero density
$dN(T) \sim (1/2\pi)\log(T/2\pi)$ becomes load-bearing, requires the
...
build against}: its digamma development carries the special values, the
recurrence $\psi(s{+}1) = \psi(s) + 1/s$, and meromorphy, but nothing at
infinity; its Stirling development has only the discrete factorial form, not the
continuous complex $\log\Gamma$ asymptotic. So the frontier is a specific absent
lemma---a genuine, Mathlib-caliber piece of new formalization---together with the
saddle-point extraction that would sit on top of it. We build the finite skeleton
to that edge and name the missing input exactly; we do not claim it. As
everywhere in this section, \texttt{conjecture1\_proved = False}: nothing here
proves or approaches RH.
