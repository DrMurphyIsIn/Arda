...
of $\Gamma$ at $1$. The route's genuine residual is therefore a \emph{different},
more elementary frontier: the higher polygamma values $\psi^{(k)}(\tfrac12)$
(exact $\zeta$-value constants, but a polygamma theory Mathlib currently lacks
entirely), a Fa\`a di Bruno assembly of the composition (algebra), and the growth
of the resulting explicit sequence (asymptotics of a known real sequence, not a
uniform complex estimate on a growing contour). We have opened this route and
planted its first exact stone; we have not walked it, and we do not claim the main
term. The honest shape of the frontier is what changed---from ``an absent complex
asymptotic'' to ``a bounded, namable polygamma build''---not the conclusion. As
