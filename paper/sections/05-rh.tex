...
These are three separate kernel theorems, \emph{deliberately not stitched} into a
claim that the count $N(T)$ determines the coefficients $\lambda_n$. Two seams
remain open and are named as such. The exhaustion limit indexes zeros by $\xi$'s
nontrivial-zero set while the finite explicit formula indexes them by $\zeta$'s
meromorphic divisor; matching the two indices (and the weight's pole at the
origin) is bookkeeping we have not done. More fundamentally, the one place where
the zero density $dN(T) \sim (1/2\pi)\log(T/2\pi)$ becomes load-bearing for the
coefficients---the Bombieri--Lagarias main term $\lambda_n \sim (n/2)\log n$---is
an Archimedean main-term extraction from the boundary integrals in the
$T \to \infty$ limit. That is genuinely new asymptotic analysis: it is present in
neither this development nor its upstream, and it is not obtainable by rewriting
the pieces we hold. We build the finite skeleton up to its edge and name exactly
what the classical bridge would require; we do not claim it. As everywhere in
this section, \texttt{conjecture1\_proved = False}: nothing here proves or
approaches RH.
