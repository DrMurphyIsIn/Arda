...
A second criterion has since joined it in the same prefix-certificate shape:
\emph{Li's criterion}. Building on an external Lean formalization of the upstream
reduction ($\mathrm{RH} \leftrightarrow \forall n,\ 0 \le
\operatorname{Re}(\text{taylorCoeff}\ \xi\ n)$; credited in the repository's
\texttt{NOTICE}), a Telperion emitter certifies finite rungs of the positivity
ladder, each from a certified positive rational lower bound on the actual $\xi$
Taylor coefficient --- the bound an explicit Arb hypothesis, the same trust seam
as every enclosure in this section. Two flags travel with it, stated as
sharply as the Borel--Carath\'eodory draft's: at this writing the emitted Lean is
verified locally but not yet in the CI compile (it awaits the upstream library as
a build dependency), so it does not yet sit on the CI-guarded list; and the
uniform $\forall n$ \emph{is} RH itself --- finitely many rungs are emphatically
...
\subsection{Toward the counting formula: shrinking the last numeric input}
...
counts is the Riemann--von Mangoldt formula, $N(T) = \theta(T)/\pi + 1 + S(T)$
--- and a program to bring that machine into the kernel is now largely built.
We report it at exact strength: every claim in this subsection is a merged,
CI-axiom-guarded theorem, and the one thing that is \emph{not} yet a theorem is
named at the end.

The classical obstruction was that Mathlib has no continuous-argument-along-a-path;
the formalization dissolves it branch-free: the continuous argument change of $f$
along a zero-avoiding path \emph{is} $\operatorname{Im} \int f'/f$, so $S(T)$ is
\emph{defined} (as the argument change of $\zeta$ along the classical route
$2 \to 2+iT \to \tfrac12+iT$), and the Riemann--Siegel $\theta$ is \emph{identified}:
the Archimedean part of that same route equals $\theta(T)$, because $\Gamma_{\mathbb R}$
has neither zeros nor poles on the rectangle (so its winding vanishes) and is real
on the real axis. On the same contour the completed-zeta argument change then
splits exactly,
\[
  \Delta_L \arg \Lambda \;=\; \theta(T) + \pi S(T),
\]
with a single hypothesis --- $\zeta \ne 0$ on the height-$T$ horizontal, the
classical ``$T$ not a zero ordinate'' caveat, carried as a hypothesis rather than
prose. Two symmetry folds (the functional-equation reflection and Schwarz
conjugation) give $\Delta_{\mathrm{box}} = 2\,\Delta_L$.

The formula's $+1$ is the pole of $\zeta$ at $s = 1$, and it too is now a theorem
rather than a citation: the companion $H(s) = (s-1)\zeta(s)$, with the removable
singularity filled by the residue, is \emph{entire}, and --- the lesson of this
campaign applied --- its zero set is not asserted but \emph{proved}:
$H(z) = 0 \leftrightarrow z \ne 1 \wedge \zeta(z) = 0$, in the axiom guard. The
box argument principle, made generic and instantiated at $H$, then yields the
meromorphic count for $\zeta$ itself: for a box containing $s = 1$, the zero count
is the boundary winding over $2\pi$ \emph{plus one}. On the classical rectangle
$[-1,2] \times [T_0,T_1]$ the same machinery gives a band-difference decomposition
of $2\pi N$ into edge data --- precisely the object each tiled band consumes ---
so this program, completed, would \emph{derive} what the ladder currently assumes,
shrinking the numeric trust surface exactly as the left-edge reflection did for
the explicit-formula box.

The honest ceiling, stated as the merged PRs state it: the literal
$N(T) = \theta(T)/\pi + 1 + S(T)$ still requires the box argument principle for
$\Lambda$ with its poles at $0$ and $1$, together with the $T_0 \to 0^+$
real-axis base --- genuine construction, not assembly, and \emph{not claimed}.
One mid-course correction is part of the record and worth citing as lived
practice for \cref{sec:vacuity}: an early route through the entire function
$\Lambda_0$ was retired when its zero set was checked against Mathlib's own
$\Lambda = \Lambda_0 - 1/s - 1/(1-s)$ (the nontrivial zeros are \emph{not}
$\Lambda_0$'s), the correction written into the corpus --- the kernel guards
statements, and the discipline guards what the statements are about.
