...
And the construction \emph{scales}: the certified height doubles by
\emph{band-stacking}. A kernel gluing lemma (\texttt{rh\_box\_two\_bands}) composes
per-band certificates---each band carrying its own small winding count and short
contour edges, so the per-band numeric certificates stay cheap and
independent---and the certificate \texttt{AllZeros\_h200} stacks a second band
$[10^{-6}, 1-10^{-6}] \times [100, 200]$ (boundary winding $N = 50$) on the first
to derive the same unweighted statement up to height $200$: all $79$ nontrivial
zeros with $0 < \operatorname{Im}\rho \le 200$ lie on the critical line---$79$
being the full classical count to that height, though as before the count is
derived from the windings, not assumed.

The caveats are stated as sharply as the claim. It is \emph{finite}---up to height
$200$, not all of $\mathbb{C}$; Turing's method verifies RH in a region, it does not
prove RH. And it is \emph{conditional on certified numeric input}: the per-band
boundary winding integers ($N = 29$ and $N = 50$), the seventy-nine exhibited
on-line zeros, the edge
non-vanishing, and the classical height floor (no nontrivial zeros below height
$\tfrac{55}{16}$) are supplied by high-precision interval (Arb) arithmetic and
...
principle, and the effective-rate inequality all in-kernel---but it does not verify
...
Turing verification up to height $200$, and it exhibits a third distinct certificate
shape (winding / argument-principle zero-counting).
