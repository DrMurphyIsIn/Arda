...
\begin{itemize}
  ...
  \item \textbf{All zeros to height \ladderHeight{}, on the line.} A complete finite
    Turing verification: the \ladderCount{} nontrivial zeros with
    $0 < \operatorname{Im}\rho \le \ladderHeight{}$ lie exactly on
    $\operatorname{Re} s = \tfrac12$, assembled from per-band
    edge-decomposition certificates composed through an unbounded-depth height
    chain, their band counts derived from Riemann--von Mangoldt edge integrals
    (each census cross-checked against the rigorous $N(T)$), with
...
clean: \emph{check the witness, don't trust the author}. A second contribution
of this paper is the method behind the first: \emph{Telperion}, a certificate
compiler for Lean~4: you
...
means the informal claim a human intended (\cref{sec:vacuity}).\footnote{The
same untrusted-generator/trusted-kernel discipline has been applied to open
problems well beyond the zeta function---an extremal-combinatorics tree-maximizer
(still open, with the missing bridge named) and a proof-complexity sum-of-squares
lower bound (mathematics due to others)---reported separately in a companion
Telperion methods paper~\cite{telperion-companion}.}
