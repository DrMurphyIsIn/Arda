...
The road ahead is marked rather than promised: running the effective machinery
on the now-proved counting formula's $S$-identity, toward Backlund's
$|S(T)| \le a\log T + b$ and a derived (rather than assumed) band census for
the height ladder; estimating the explicit
formula's boundary terms and sending the box to its limits; extending the
Turing verification beyond height $4000$, which the height-chain architecture
makes a driver exercise rather than a proof (the repository carries a
measured-cost route to height $10^6$, marked rather than promised); and letting the finite identity
