\section{Methods: the Telperion certificate engine}
\label{sec:system}
...
Telperion is not tied to one kind of certificate. It carries well over a hundred
emitters---one per certificate \emph{shape}, and the catalog grows as each
campaign distils its reusable moves---spanning a broad slice of concrete
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Analytic & zero-free-region atoms, half-plane/disk bounds, transcendental
      enclosures, box-robust positivity, degree-2 hyperbolicity (real-rootedness),
      two-scale disk separation, far-pole rational-sum bounds, Herglotz lower bounds \\
    \bottomrule
  \end{tabular}
...
continuous integration compiles the emitted Lean against pinned Mathlib on every
change. The full regenerate-and-recheck recipe is Appendix~\ref{sec:reproducibility}.
