...
% Turing-ladder milestone (update in ONE place as the campaign climbs)
\newcommand{\ladderHeight}{440{,}000}
\newcommand{\ladderCount}{711{,}253}
...
certified); a complete finite Turing verification that \emph{all \ladderCount{}
nontrivial zeros up to height \ladderHeight{} lie on the critical line},
band-tiled on edge-decomposition certificates with a uniform, documented numeric
trust boundary and a kernel statement-match gate on every band---a verification
whose diffraction pattern, computed from the certified ordinates, visibly
reproduces Dyson's quasicrystal (Bragg peaks at every $m\log p$, matching
Landau's theorem to four significant figures); the \emph{Riemann--von Mangoldt counting
...
\input{sections/10-epilogue}

\appendix
\input{sections/A-reproducibility}
\input{sections/B-compendium}
