...
| 8 | Capped-joint g-step arc: achievability correction (unconstrained Case-2 false on `μ∈(1/2,1)`; non-leaf messages `μ≤1/2`) + kernel-checked closure of the config g-step at every arity via the abstract cavity g-lemma `gV_le` | PARTIAL (kernel-checked sub-result of open BG; closure `gstep_le_one_achievable` on the PR #20 branch, pending merge) | single-hub wiring of the ≤-half; BG itself unproved | **novelty-uncertain** — *needs lit check* | `proof/formalization/R3Cert/CappedJointAchievable.lean`, `telperion/examples/g1_floors/lean/GLemma.lean` |
