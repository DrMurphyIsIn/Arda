...
- [`telperion/PROOF_STATUS.md`](telperion/PROOF_STATUS.md) — the honest map: proven, ruled-out (with reasons), and the one live lead.
...
| **R1 single-hub** | master inequality `arm_maximal`: base (leaf) + chains + branching *analytic* steps (g-lemma unimodality over ℝ; two rational leaves) | analytic steps **PROVEN**; inductive **wiring OPEN** | PROOF_ASSEMBLY §R1 |
...
every analytic step are individually sound, awaiting (1) the R1 branch-induction
wiring and (2) the R2 multi-hub maximality.
...
lemmas (42+55 certificates). Open there: the honest-conditional `R7'` assembly
and independent review (`proof/docs/design/R7_ARCHITECTURE.md`).
...
| **Emitters / shapes** | Direct Pólya, Bilinear Box, ExactFact/Identity, ℕ-reparam (`Nat.cast_sub`), case-dispatch (`interval_cases`) assembly, subdivision glue, variable-map, dichotomy (`le_total`), symbolic-tail (`∀K≥K₀`), custom | `emit.py`, `emit_facts.py`, `emit_adapters.py`, `tails.py`, `varmap.py`, `dichotomy.py` |
...
| **Diagnostics / honesty** | `diagnose` triage (FALSE-with-witness / NOT_POLYA / CERTIFIABLE), tie-variety + margins, counterexample hunt (incl. GA/MAP-Elites transfer), relax probe | `diagnose.py`, `margins.py`, `hunt.py`, `counterexample_hunt.py`, `relax.py` |
...
Version `0.1.3`. Core dependency: sympy. The BG graph-certificate modules
additionally use networkx (lazy import; install the `graph`/`dev` extra).
